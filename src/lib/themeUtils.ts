    ...
    const saveToDb = async () => {
      try {
        if (window.ipc?.invoke) {
          ...
          logger.info('[themeUtils] Theme saved to database');
        } else if (window.electronAPI?.invoke) {
          await window.electronAPI.invoke('DB:SET_SETTING', { key, value: themeJson });
          logger.info('[themeUtils] Theme saved to database');
        }
      } catch (error) {
        logger.warn('[themeUtils] Failed to save theme to database (using localStorage):', error);
      }
    };
...
export function loadTheme(key = 'user_theme') {
  ...
  try {
    ...
    if (raw) {
      ...
      logger.info('[themeUtils] Theme loaded from localStorage');
      return theme;
    }
  } catch {
    ...
    try {
      if (window.ipc?.invoke) {
        ...
        if (result?.success && result.settings?.[key]) {
          ...
          logger.info('[themeUtils] Theme loaded from database');
          return theme;
        }
      } else if (window.electronAPI?.invoke) {
      ...
      logger.warn('[themeUtils] Failed to load theme from database, trying localStorage:', error);
    }
  }
