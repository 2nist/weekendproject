...
// ✅ PERFORMANCE FIX: React.memo with shallow comparison
// Still provides major optimization by preventing re-renders when parent re-renders
export const SectionContainer = React.memo<SectionContainerProps>(
  ({
    ...
    // ✅ PERFORMANCE FIX: useMemo prevents recalculating active beat on every render
    // This eliminates 24,000 loop iterations/sec (8 sections × 60 FPS × 50 beats)
    ...
    // ✅ PERFORMANCE FIX: Debounce auto-scroll to reduce from 60 FPS to 10 FPS
    // Eliminates 50 unnecessary scrollIntoView() calls per second
    useEffect(() => {
    ...
    // ✅ PERFORMANCE FIX: useMemo for measure width calculation
    const measureWidth = React.useMemo(() => {
      const cardWidthPx = 32;
      const gapPx = 8;
      const measurePaddingPx = 16;
