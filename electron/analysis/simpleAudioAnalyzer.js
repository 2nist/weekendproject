...
async function analyzeAudioSimple(filePath, progressCallback = () => {}) {
  ...
  console.log('Simple analyzer: Processing', samplesArray.length, 'samples, duration:', duration, 'seconds');
  ...
  console.log('Simple analyzer: Beat detection complete -', beatTimestamps.length, 'beats, tempo:', tempo);
  progressCallback(40);
  await new Promise(resolve => setImmediate(resolve));
  ...
  console.log('Simple analyzer: Chroma extraction complete -', chromaFrames.length, 'frames');
  progressCallback(60);
  await new Promise(resolve => setImmediate(resolve));
  ...
  for (let i = 0; i < beatTimestamps.length; i++) {
    ...
    const nearestFrame = chromaFrames.reduce((prev, curr) => {
      return Math.abs(curr.timestamp - beatTime) < Math.abs(prev.timestamp - beatTime) ? curr : prev;
    });
  ...
  console.log('Simple analyzer: Event processing complete -', events.length, 'events');
  ...
  if (!result.linear_analysis.events || result.linear_analysis.events.length === 0) {
    console.warn('Simple analyzer: No events detected, creating placeholder event');
  ...
  if (!result.linear_analysis.beat_grid.beat_timestamps || result.linear_analysis.beat_grid.beat_timestamps.length === 0) {
    console.warn('Simple analyzer: No beats detected, creating placeholder beats');
  ...
  if (!result.linear_analysis.chroma_frames || result.linear_analysis.chroma_frames.length === 0) {
    ...
    result.linear_analysis.chroma_frames = Array(frameCount).fill(null).map((_, i) => ({
      timestamp: i * 0.1,
