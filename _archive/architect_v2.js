...
function detectNoveltyMultiScale(matrixObj, opts = {}) {
  ...
  const scales = opts.noveltyKernelSizes ? 
    ...
    [
      ...
      { size: 9, weight: 0.50, label: 'section' },
...
function adaptivePeakPicking(noveltyCurve, opts = {}) {
  ...
  for (let i = windowFrames; i < n - windowFrames; i++) {
    ...
    if (noveltyCurve[i] <= noveltyCurve[i - 1] || 
    ...
    for (let bj = bi; bj < n; bj += blockSize) {
      ...
      for (let i = bi; i < iEnd; i++) {
        for (let j = Math.max(bj, i); j < jEnd; j++) {
          ...
          const vRms = rms[i] !== undefined && rms[j] !== undefined
            ? 1.0 - Math.min(1.0, Math.abs(rms[i] - rms[j]))
            : 1.0;
          const vFlux = flux[i] !== undefined && flux[j] !== undefined
...
function computeBeatSynchronousChroma(linear) {
  ...
  const frameHopSeconds = linear.metadata?.frame_hop_seconds || 
...
function labelSections(sections, clusters, linear) {
  ...
  console.log(`Architect V2: Label distribution - ${JSON.stringify(
...
function mergeSimilarSections(sections, chromaFrames, mfccFrames, opts = {}) {
  ...
  console.log(`Architect V2: Merged ${mergeCount} sections (${sections.length} -> ${result.length})`);
  return result;
}
  ...
  if (validation.suggestions && validation.suggestions.length > 0) {
    ...
    for (const suggestion of validation.suggestions) {
      if (suggestion.type === 'relabel' && suggestion.confidence > 0.7) {
        ...
        if (section) {
          console.log(`  Relabeling section ${suggestion.sectionIndex}: ${section.label} -> ${suggestion.newLabel}`);
...
async function analyzeStructure(linear, progressCallback = () => {}, opts = {}) {
  ...
  const peaks = adaptivePeakPicking(noveltyResult.noveltyCurve, {
    ...
    localWindowSec: 10.0,
    minPeakDistance: Math.round((adaptiveOpts.minSectionSeconds || 1.5) / FRAME_HOP_SECONDS / downsampleFactor)
  });
  ...
  const { sections, clusters } = clusterSections(fullSSM, boundaries, new Set(), false, adaptiveOpts.clusterSimilarity || 0.6);
